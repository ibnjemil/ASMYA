...
export default function ChatView({ chat, onBack }: ChatViewProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (currentReply) {
        const quoteText = stripQuotePrefix(currentReply.content).substring(0, 80)
      ...
      const tempMsg: MessageInfo = {
        ...
        content: text || msgContent, mediaUrl, createdAt: new Date().toISOString(),
        sender: { id: user.id, username: user.username, displayName: user.displayName, avatarUrl: user.avatarUrl, role: user.role, side: user.side }
      }
